...
export class Seed {
  ...
  has_content() {
    return !!(
      Object.keys(this.obj_tree).length ||
      Object.keys(this.attr_tree).length ||
      this.obj_vocab.length ||
      this.attr_vocab.length ||
      this.conventions.length ||
      this.reference_k
    );
  }
...
export function loadSeed(filePath, fs) {
  try {
...
export function saveSeed(seed, filePath, fs) {
  fs.writeFileSync(filePath, JSON.stringify(seed.to_dict(), null, 2));
}
